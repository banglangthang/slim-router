# Slim Router Architecture

_Last updated: 2026-02-06_

## Executive Summary

Slim Router is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[Slim Router Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.slim-router/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.slim-router/usage.json`, `~/.slim-router/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.slim-router/db.json`)
- usage stats: `~/.slim-router/usage.json`
- request log lines: `~/.slim-router/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Slim Router Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.slim-router` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/slim-router && npm run build`
- Build Docker image: `cd /root/dev/slim-router && docker build -t slim-router .`
