...
// Collect PIDs of all slim-router-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from slim-router install dir or wrapper cli.js
        const isAppProcess = lower.includes("slim-router") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("slim-router") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "slim-router");
  }
  return path.join(os.homedir(), ".slim-router");
}
...
// Resolve npx/slim-router binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx slim-router` — works regardless of global bin path changes after npm i -g
