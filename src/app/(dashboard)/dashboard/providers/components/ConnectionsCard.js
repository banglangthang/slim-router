...
import { Card, Badge, Button, Modal, Toggle, EditConnectionModal } from "@/shared/components";
...
function ConnectionRow({ connection, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onEdit, onDelete }) {
  ...
  const proxyBadgeVariant = hasLegacyProxy ? "error" : "default";
          ...
          <div className="pt-6">
            ...
            {provider === "github" && (
              <Button
                onClick={async () => {
                  try {
                    const res = await fetch("/api/gh-token", { method: "POST" });
                    const data = await res.json();
                    if (data.token) {
                      setFormData({ ...formData, apiKey: data.token });
                    } else {
                      alert(data.error || "Failed to get token");
                    }
                  } catch (err) {
                    alert("Failed to fetch GitHub token. Is gh CLI installed?");
                  }
                }}
                variant="secondary"
                className="ml-2"
