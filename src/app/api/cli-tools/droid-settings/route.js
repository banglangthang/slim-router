...
// Check if settings has Slim Router customModels
const hasSlim RouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Slim Router"));
};
...
// POST - Update Slim Router customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Slim Router configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Slim Router"));
...
// DELETE - Remove Slim Router customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Slim Router customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Slim Router"));
