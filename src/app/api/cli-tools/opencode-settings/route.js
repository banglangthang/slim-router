...
const hasSlimRouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["slim-router"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["slim-router"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasSlimRouter: hasSlimRouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("slim-router/") ? config.model.replace(/^slim-router\//, "") : null,
...
// POST - Apply Slim Router as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_slim_router";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing slim-router provider entry and its models
    const existingProvider = config.provider["slim-router"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["slim-router"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `slim-router/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `slim-router/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("slim-router/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Slim Router provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["slim-router"]?.models) {
      delete config.provider["slim-router"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["slim-router"].models).length === 0) {
        delete config.provider["slim-router"];
        if (config.model?.startsWith("slim-router/")) delete config.model;
      } else if (config.model === `slim-router/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["slim-router"].models);
        config.model = `slim-router/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire slim-router provider
      if (config.provider) delete config.provider["slim-router"];
      if (config.model?.startsWith("slim-router/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("slim-router/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Slim Router settings removed from OpenCode",
    });
  } catch (error) {
