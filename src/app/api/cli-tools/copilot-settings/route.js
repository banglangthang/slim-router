...
const hasSlim RouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Slim Router");
};

const getSlim RouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Slim Router") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getSlim RouterEntry(config);
...
// POST - Apply Slim Router config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_slim-router";
    ...
    // Replace existing Slim Router entry or append
    const idx = config.findIndex((e) => e.name === "Slim Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Slim Router entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Slim Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Slim Router removed from Copilot config",
    });
  } catch (error) {
