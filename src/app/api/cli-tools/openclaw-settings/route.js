...
// Check if settings has Slim Router config
const hasSlim RouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["slim-router"];
};

// Read per-agent models.json and return current model id (without "slim-router/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["slim-router"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasSlim Router: hasSlim RouterConfig(settings),
...
const writeAgentModels = async (agentDir, model, baseUrl, apiKey) => {
  ...
  existing.providers["slim-router"] = {
    baseUrl,
...
// POST - Update Slim Router settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `slim-router/${model}`;

    // Remove all old slim-router/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("slim-router/"))
    ...
    // Add fresh slim-router models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`slim-router/${m}`] = {};
    });

    // Remove old slim-router model from each agent in agents.list
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (agent.model?.startsWith("slim-router/")) {
    ...
    // Update models.providers.slim-router with all models
    settings.models.providers["slim-router"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `slim-router/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Slim Router settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Slim Router from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["slim-router"];
    ...
    // Remove slim-router models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("slim-router/"));
    ...
    // Reset agents.defaults.model.primary if it uses slim-router
    if (settings.agents?.defaults?.model?.primary?.startsWith("slim-router/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "Slim Router settings removed successfully",
    });
  } catch (error) {
